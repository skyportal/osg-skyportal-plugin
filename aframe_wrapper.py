...
def run(inputs: dict) -> dict:
    from aframe_bridge import run_from_skyportal_inputs  # shipped per-job
...
def bundle_for_skyportal(result: dict) -> dict:
    ...
    if result.get("status") == "failure":
        return {
            "status": "failure",
            "message": result.get("message", "aframe failed"),
            "analysis": {},
        }
    ...
    return {
        "status": "success",
        "message": result.get("message", "aframe search complete"),
        "analysis": analysis,
    }
