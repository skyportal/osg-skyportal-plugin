"""
SNID-SAGE wrapper — runs inside the SNID-SAGE runtime image on an OSG worker.

Contract (same as ngsf_wrapper):
- Reads ``inputs.json`` from cwd (staged by HTCondor file transfer).
- Runs a spectral classification via ``snid_bridge`` (shipped alongside this file).
- Bundles the result as SkyPortal's analysis-callback JSON
  ``{status, message, analysis: {results, plots, annotations}}``; the
  annotations carry the best-match type, subtype, redshift and age.
...
def run(inputs: dict) -> dict:
    from snid_bridge import run_from_skyportal_inputs  # shipped per-job
...
def bundle_for_skyportal(result: dict) -> dict:
    ...
    if result.get("status") == "failure":
        return {
            "status": "failure",
            "message": result.get("message", "SNID-SAGE failed"),
            "analysis": {},
        }
