"""pygrb wrapper — runs inside the pycbc runtime image on an OSG worker.

Contract (same as fiesta_wrapper/mosfit_wrapper):
- Reads ``inputs.json`` from cwd (staged by HTCondor file transfer).
- Runs a coherent GW targeted search via ``pygrb_bridge`` (shipped alongside).
- Bundles the result as SkyPortal's analysis-callback JSON
  ``{status, message, analysis: {model_name, results, plots}}``.
- Optionally PUTs the bundle to ``$OSDF_OUTPUT_URL``; always writes it to stdout
  so the plugin's poller can scrape it.

Unlike the light-curve wrappers there is no photometry to materialize: the inputs
are a trigger time, sky position, and detector list.
"""
...
def run(inputs: dict) -> dict:
    from pygrb_bridge import run_from_skyportal_inputs  # shipped per-job
...
def bundle_for_skyportal(result: dict) -> dict:
    """Pack the bridge output into SkyPortal's analysis-callback schema: the JSON
    result blob (coherent SNR, best GPS, per-detector peaks) plus the trigger-
    distribution plot."""
    if result.get("status") == "failure":
        return {
            "status": "failure",
            "message": result.get("message", "pygrb failed"),
            "analysis": {},
        }

    analysis: dict = {"model_name": result.get("source", "pygrb")}
    ...
    return {
        "status": "success",
        "message": result.get("message", "pygrb search complete"),
        "analysis": analysis,
    }
