"""
ALMA wrapper — runs inside the ALMA runtime image on an OSG worker.

Contract (same as ngsf_wrapper):
- Reads ``inputs.json`` from cwd (staged by HTCondor file transfer).
- Reduces the staged ALMA products via ``alma_bridge`` (shipped alongside).
- Bundles the result as SkyPortal's analysis-callback JSON
  ``{status, message, analysis: {results, plots, annotations}}``; the
  plots are the moment-0 map and the extracted spectrum for each cube.
...
def bundle_for_skyportal(result: dict) -> dict:
    ...
    if result.get("status") == "failure":
        return {
            "status": "failure",
            "message": result.get("message", "ALMA reduction failed"),
            "analysis": {},
        }
    ...
    return {
        "status": "success",
        "message": result.get("message", "ALMA reduction complete"),
        "analysis": analysis,
    }
